...
predicted_Marker_fun_F <- function(st, Dat) {
  ...
  fit <- lmer(yij ~ start + (1|id) + (1|cluster), data = Dat, control = lmerControl(optimizer = "bobyqa"))
...
predicted_Marker_fun_T <- function(st, Dat) {
  ds_subset <- Dat[Dat$ST >= st & Dat$A0 <= st & Dat$start <= st, ]
  fit <- lmer(yij ~ start + (1|id) + (0+start|id) + (1|cluster), data = Dat, REML = FALSE, control = lmerControl(optimizer = "bobyqa"))
  newSubset <- data.frame(id = unique(ds_subset$id), 
                          cluster = subset(ds_subset, !duplicated(id))$cluster,
                          A0 = subset(ds_subset, !duplicated(id))$A0,
                          ST = subset(ds_subset, !duplicated(id))$ST,
...
Lik_ORC_fun_F <- function(p, dat1, predicted_Marker, di, e, GG) {
...
Lik_ORC_fun_T <- function(p, dat1, predicted_Marker, di, e, GG) {
  cumhazStart <- aggregate(out ~ cluster, transform(dat1, out = exp(p[1]) * (start^exp(p[2])) * exp(predicted_Marker * p[4])), sum)[,2]
  cumhazEntry <- aggregate(out ~ cluster, transform(subset(dat1, !duplicated(id)), out = exp(p[1]) * (start^exp(p[2])) * exp(predicted_Marker * p[4])), sum)[,2]
  cumhazStop <- aggregate(out ~ cluster, transform(dat1, out = exp(p[1]) * (stop^exp(p[2])) * exp(predicted_Marker * p[4])), sum)[,2]
  lnhaz <- aggregate(out ~ cluster, transform(dat1, out = status * (predicted_Marker * p[4] + log(exp(p[1]) * exp(p[2]) * stop^(exp(p[2]) - 1)))), sum)[,2]
...
# ---- Main Function ----
fit_ORC <- function(data, cluster, id, inip, Marker, MarkerTime, status,  SurvTime, A0, hess, randslope) {
  ...
  # Choose correct predicted_Marker function
  ...
  LMM_Output <- map_dfr(tt[1:(nt-1)], predicted_Marker)
  ...
  # Choose correct Lik_ORC function
  Lik_ORC <- if (randslope) {
    function(p) Lik_ORC_fun_T(p, dat1, dat1$predicted_Marker, di, e, GG)
  } else {
    function(p) Lik_ORC_fun_F(p, dat1, dat1$predicted_Marker, di, e, GG)
  }
  
  system.time(  t2 <- nlminb(inip, Lik_ORC, hessian = hess, control = list(rel.tol = 1e-6, trace=1))  )
...
attach(DatH)
fit_ORC ( data=DatH, cluster = TwinID, id=id, inip=c(  log(0.01), log(2),0.3, 2 ) ,
